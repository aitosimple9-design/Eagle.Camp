...
export function calculateVisibleTotal(state) {
    ...
    keys.forEach(k => {
        const hidden = Array.isArray(state.hiddenKeys)
            ? state.hiddenKeys.includes(k)
            : state.hiddenKeys.has(k);
        if (!hidden) visible += (breakdown[k] || 0);
    });
    ...
    if (Array.isArray(state.extraBonuses)) {
        state.extraBonuses.forEach(b => {
            ...
            const hidden = Array.isArray(state.hiddenKeys)
                ? state.hiddenKeys.includes(key)
                : state.hiddenKeys.has(key);
            if (!hidden) {
                const monthly = safeNum(b.amountVND) / 1_000_000;
                visible += monthly;
            }
